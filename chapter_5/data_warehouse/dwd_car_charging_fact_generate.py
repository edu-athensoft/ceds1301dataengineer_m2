"""
dwd_car_charging_fact collection tools
"""
import time
...
from chapter_5.model.dwd_car_charging_model import CarChargingModel
from chapter_5.config import project_config as conf

logger = logging_util.init_logger('dwd_car_charging_fact')
logger.info('dwd_car_charging_fact data collection starts...')
...
metadata_util.check_table_exists_and_create(
    ...
    tb_name=conf.metadata_dwd_car_charging_monitor_table_name,
    tb_cols=conf.metadata_dwd_car_charging_monitor_table_create_cols
)
# Query the maximum updateAt value of the last collected record in the product collection metadata table
sql = f"select max(time_record) from {conf.metadata_dwd_car_charging_monitor_table_name}"
car_charging_monitor_table_results = metadata_util.query(sql)

if car_charging_monitor_table_results[0][0]:
    car_charging_max_time = car_charging_monitor_table_results[0][0]
else:
    car_charging_max_time = None
...
if not source_util.check_table_exists(conf.source_data_db, conf.source_ods_car_data_logs_table_name):
...
if not car_charging_max_time:
    sql = f"select {conf.target_dwd_car_charging_fact_table_insert_cols[1:-1]} \
    from {conf.source_ods_car_data_logs_table_name} where car_status=2 and charge_status=1 order by date_ts ;"
else:
    sql = f"select {conf.target_dwd_car_charging_fact_table_insert_cols[1:-1]} \
    from {conf.source_ods_car_data_logs_table_name} where date_ts > '{car_charging_max_time}' \
    and car_status=2 and charge_status=1 order by date_ts ;"

source_car_charging_table_results = source_util.query(sql)
# Determine the number of collected data entries. If the number of entries is 0, exit the program
if not source_car_charging_table_results:
...
target_util.check_table_exists_and_create(
    ...
    tb_name=conf.target_dwd_car_charging_fact_table_name,
    tb_cols=conf.target_dwd_car_charging_fact_table_create_cols
)
...
for row_data in source_car_charging_table_results:
    try:
        model = CarChargingModel(row_data)
        ...
        if data_count % conf.batch_commit == 0:
            ...
            sql = sql_util.get_metadata_sql(data_count, conf.batch_commit, model,
                                            conf.metadata_dwd_car_charging_monitor_table_name,
                                            conf.metadata_dwd_car_charging_monitor_table_insert_cols)
