"""
ads_car_electric_driving_stats_m collection tools
"""
import time

from chapter_5.util import mysql_util
from chapter_5.util import logging_util
from chapter_5.model.ads_car_driving_model import CarDrivingStatsModel
from chapter_5.config import project_config as conf

logger = logging_util.init_logger('ads_car_electric_driving_stats_m')
logger.info('ads_car_electric_driving_stats_m data collection starts...')
...
if not source_util.check_table_exists(conf.source_data_db, conf.source_dws_car_electric_driving_d_table_name):
...
source_car_electric_driving_stats_table_results = source_util.query(conf.target_ads_car_electric_driving_stats_m_table_select_sql)
# Determine the number of collected data entries. If the number of entries is 0, exit the program
if not source_car_electric_driving_stats_table_results:
...
target_util.check_table_exists_and_create(
    ...
    tb_name=conf.target_ads_car_electric_driving_stats_m_table_name,
    tb_cols=conf.target_ads_car_electric_driving_stats_m_table_create_cols
)

source_util.query(conf.target_ads_car_electric_driving_stats_m_table_delete_sql)
...
for row_data in source_car_electric_driving_stats_table_results:
    try:
