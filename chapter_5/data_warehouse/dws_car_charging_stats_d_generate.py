"""
dws_car_charging_stats_d collection tools
"""
import time
...
from chapter_5.model.dws_car_charging_model import CarChargingStatsModel
from chapter_5.config import project_config as conf

logger = logging_util.init_logger('dws_car_charging_stats_d')
logger.info('dws_car_charging_stats_d data collection starts...')
...
metadata_util.check_table_exists_and_create(
    ...
    tb_name=conf.metadata_dws_car_charging_stats_monitor_table_name,
    tb_cols=conf.metadata_dws_car_charging_stats_monitor_table_create_cols
)
# Query the maximum updateAt value of the last collected record in the product collection metadata table
sql = f"select max(time_record) from {conf.metadata_dws_car_charging_stats_monitor_table_name} "
...
if not source_util.check_table_exists(conf.source_data_db, conf.source_dwd_car_charging_fact_table_name):
...
if not car_charging_max_time:
    sql = f"select {conf.target_dws_car_charging_stats_d_table_select_cols[1:-1]} \
    from {conf.source_dwd_car_charging_fact_table_name} \
    group by vin,date(date_ts) \
    order by date(date_ts) ;"
else:
    sql = f"select {conf.target_dws_car_charging_stats_d_table_select_cols[1:-1]} \
    from {conf.source_dwd_car_charging_fact_table_name} \
    where date(date_ts) > '{car_charging_max_time}' \
    group by vin,date(date_ts) \
    order by date(date_ts) ;"

source_car_charging_stats_table_results = source_util.query(sql)
...
target_util.check_table_exists_and_create(
    ...
    tb_name=conf.target_dws_car_charging_stats_d_table_name,
    tb_cols=conf.target_dws_car_charging_stats_d_table_create_cols
)
    ...
    try:
        ...
        if data_count % conf.batch_commit == 0:
            ...
            sql = sql_util.get_metadata_sql(data_count, conf.batch_commit, model,
                                            conf.metadata_dws_car_charging_stats_monitor_table_name,
                                            conf.metadata_dws_car_charging_stats_monitor_table_insert_cols)
