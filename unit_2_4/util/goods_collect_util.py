"""
Goods collection tools
"""
import time

from unit_2_4.util import mysql_util
from unit_2_4.util import logging_util
from unit_2_4.model.goods_model import GoodsModel
from unit_2_4.config import project_config as conf

logger = logging_util.init_logger('goods')
logger.info('Goods data collection starts...')
...
metadata_util.check_table_exists_and_create(
    ...
    tb_name=conf.metadata_goods_table_name,
    tb_cols=conf.metadata_goods_table_create_cols
)
# Query the maximum updateAt value of the last collected record in the product collection metadata table
sql = f"select max(time_record) from {conf.metadata_goods_table_name}"
goods_monitor_table_results = metadata_util.query(sql)

if goods_monitor_table_results[0][0]:
    goods_max_time = goods_monitor_table_results[0][0]
else:
    goods_max_time = None
...
if not source_util.check_table_exists(conf.source_data_db, conf.source_goods_table_name):
...
if not goods_max_time:
    sql = f"select * from {conf.source_goods_table_name} order by updateAt;"
else:
    sql = f"select * from {conf.source_goods_table_name} where updateAt > '{goods_max_time}' order by updateAt;"

source_goods_table_results = source_util.query(sql)
# Determine the number of collected data entries. If the number of entries is 0, exit the program
if not source_goods_table_results:
...
target_util.check_table_exists_and_create(
    ...
    tb_name=conf.target_goods_table_name,
    tb_cols=conf.target_goods_table_create_cols
)
...
for row_data in source_goods_table_results:
    data_count += 1
    try:
        model = GoodsModel(row_data)
    ...
    if data_count % 100 == 0:
        ...
        sql = f"insert into {conf.metadata_goods_table_name}(time_record, gather_line_count) values (" \
