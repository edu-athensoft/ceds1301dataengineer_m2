...
from chapter_2_3_2_db.util import mysql_util
from chapter_2_3_2_db.util import logging_util
from chapter_2_3_2_db.model.products_model import ProductsModel
from chapter_2_3_2_db.config import project_config as conf
...
for row_data in source_products_table_results:
    ...
    if data_buffer_count % 100 == 0:
        ...
        logger.info(f'{data_buffer_count} records have been collected successfully!')
...
if model == None or data_buffer_count == 0:
    ...
    sql = f"insert into {conf.metadata_products_table_name}(time_record, gather_line_count) values (" \
          f"'{model.update_at}', {data_buffer_count % 100});"
    metadata_util.insert_sql(sql)

end = time.time()
logger.info(f'a total of {data_buffer_count} records were collected and the total time consumed was {end - start}s')
