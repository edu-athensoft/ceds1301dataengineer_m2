"""
order_payments collection tools
"""
import time

from chapter_3_2_3.util import mysql_util
from chapter_3_2_3.util import logging_util
from chapter_3_2_3.model.order_payments_model import OrderPaymentsModel
from chapter_3_2_3.config import project_config as conf

logger = logging_util.init_logger('order_payments_db')
logger.info('order_payments data collection starts...')
...
metadata_util.check_table_exists_and_create(
    ...
    tb_name=conf.metadata_order_payments_table_name,
    tb_cols=conf.metadata_order_payments_table_create_cols
)
# Query the maximum updateAt value of the last collected record in the order_payments collection metadata table
sql = f"select max(time_record) from {conf.metadata_order_payments_table_name}"
order_payments_monitor_table_results = metadata_util.query(sql)

if order_payments_monitor_table_results[0][0]:
    order_payments_max_time = order_payments_monitor_table_results[0][0]
else:
    order_payments_max_time = None
...
if not source_util.check_table_exists(conf.source_data_db, conf.source_order_payments_table_name):
    logger.error('Sorry, the order_payments source database table you want to access does not exist!')
    exit('Sorry, the order_payments source database table you want to access does not exist!')
...
if not order_payments_max_time:
    sql = f"select * from {conf.source_order_payments_table_name} order by payment_date;"
else:
    sql = f"select * from {conf.source_order_payments_table_name} where payment_date > '{order_payments_max_time}' order by payment_date;"

source_order_payments_table_results = source_util.query(sql)
# Determine the number of collected data entries. If the number of entries is 0, exit the program
if not source_order_payments_table_results:
    logger.info('Sorry, there is no order_payments data to collect!')
    exit('Sorry, there is no order_payments data to collect!')
...
target_util.check_table_exists_and_create(
    ...
    tb_name=conf.target_order_payments_table_name,
    tb_cols=conf.target_order_payments_table_create_cols
)


# Transaction processing function implementation
# 1. Open the transaction
# 2. Submit a transaction every 100 cycles (read 100 order_payments data)
# 3. If the insertion fails, roll back the transaction
# 4. Define a data_count variable to record the number of order_payments read
data_count = 0
...
for row_data in source_order_payments_table_results:
    data_count += 1
    try:
        model = OrderPaymentsModel(row_data)
    ...
    # Is the number of order_payments read a multiple of 100， Manually submit a transaction
    if data_count % 100 == 0:
        ...
        sql = f"insert into {conf.metadata_order_payments_table_name}(time_record, gather_line_count) values (" \
